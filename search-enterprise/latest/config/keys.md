...
2. Copy the files under `/keystores/solr` to the Alfresco Search Enterprise install location:
...
5. Change the SSL properties in `<SOLR_HOME>/solrhome/templates/rerank/conf/solrcore.properties`.
    ...
    The `rerank` template is used to generate the `alfresco` and `archive` Solr cores when you first run Alfresco Search Enterprise.
